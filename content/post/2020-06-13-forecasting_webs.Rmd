---
title: "Web Scraping y Forecasting"
date: 2020-06-13
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Introducción

En esta oportunidad quiero trabajar con dos desarrollos muy interesantes a nivel de implementación, la primera tiene que ver con un web scraping, dado un proyecto en el cual estoy trabajando con una persona que admiro demasiado, el doctor [Carlos Ospino](https://www.linkedin.com/in/cospino/) y por otro lado respondiendo a un reto, sobre trading financiero por parte de [Daniel Cordoba](https://www.linkedin.com/in/daniel-alejandro-cordoba-pulido-6b9199103/), el cual me a obligado a pensar fuera de la caja con una serie de metodologías. Por lo anterior el trabajo que hoy desarrollaré es intentar predecir el precio del dolar, con modelos de memoria corta.


La fuente de datos es [investing](https://es.investing.com/currencies/usd-cop-historical-data), y la bibliografía que consulte para esta post es la siguiente:

* [Forecasting: Principles and Practices](https://otexts.com/fpp2/), de Rob J Hyndman
* [time series analysis and its applications](https://www.springer.com/gp/book/9783319524511#:~:text=With%20R%20Examples,into%20a%20more%20practical%20context), de Robert H Shumway

Dicho lo anterior, manos a la obra!

**Anotación especial**: No trabajaré con variables macroeconómicas, dado que este es un ejercicio, más junto a Daniel Cordoba, estamos desarrollando un proyecto de un modelo de Machine Learning for Time Series en R, el cual estará disponible en algunos meses.


## EDA

```{r, echo=FALSE, cache=FALSE, message=FALSE, warning=FALSE}
library(tidyverse);
library(fpp2);
library(TSA);
library(rvest);
library(httr);
library(fable);
library(forecast);
library(ggrepel);
library(astsa)
theme_set(theme_classic())
```


### Web Scraping

hacer web scraping es un arte prácticamente, hay que tener presente cómo esta el protocolo http, si es 200 se puede trabajar perfectamente, de lo contrario, tocaría hacer un par de cosas sobre el texto que intenta leer a nivel de transformación `.html`. Si desea tener más información al respecto le recomiendo ver el siguiente [link](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes), por otro lado, si desea integrar y mantener un deploy sobre esto, por experiencia le recomiendo trabajar con Flask, hace parte de las buenas prácticas.


Para revisar el `status code`, puede ejecutar el siguiente código

```{r}
url<-"https://es.investing.com/currencies/usd-cop-historical-data"
url%>%  
  GET()%>%
  http_status()
```

Ahora, voy a descargar la tabla de mi interés y empezar a desarrollar un corto análisis.


```{r}
url<-"https://es.investing.com/currencies/usd-cop-historical-data"

dolar_table<-url%>%
  read_html()%>%
  html_node('#curr_table')%>%
  html_table()%>%
  tbl_df()

dolar_table

```

Bueno, hay que hacerle un par de modificaciones a la tabla para poder trabajar con ella.

```{r}
Serie<-dolar_table%>%
  rename('Cierre'='Último')%>%
  mutate(Fecha=(gsub('\\.','-',Fecha)),
         Apertura=gsub("\\.","",Apertura),
         Apertura=gsub(",",".",Apertura),
         Apertura=as.numeric(Apertura),
         Cierre=gsub("\\.","",Cierre),
         Cierre=gsub(",",".",Cierre),
         Cierre=as.numeric(Cierre))%>%
  select(c(Fecha,Apertura,Cierre))


Serie$Fecha<-lubridate::dmy(Serie$Fecha)
Serie
```

Para este análisis me basaré puntualmente en el cierre

```{r, echo=FALSE, cache=FALSE}
Serie%>%
  ggplot(aes(Fecha,Cierre))+
  geom_point(color='blue')+
  geom_label_repel(aes(label=Cierre),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50',
                  size=2)+
  geom_line()+
  labs(title = 'Cierre del Dolar en Colombia',
       subtitle = 'Entre el período 2020-05-13 y 2020-06-12')
```


## Build a model

El comportamiento del dólar ha sido bastante dinámico, por lo tanto trabajar con un modelo de memoria corta puede ser un camino correcto, aunque no estoy muy seguro que el mejor, pero veamos esto a través de los resultados.

Lo primero que quiero evaluar es el tipo de comportamiento de los datos, a través de la tendencia. Lo más importante  a destacar es que se trata de una regresión Loess o local[^1], por lo cual entender los segmentos de tiempo será la clave de este asunto.

```{r, warning=FALSE, message=FALSE}
Serie%>%
  ggplot(aes(Fecha,Cierre))+
  geom_smooth(se=FALSE)+
  labs(title = 'Tendencia del comportamiento del dolar',
       subtitle = 'a través de la estructura de una regresión mixta')
```

Me llama bastante la atención del mínimo local de $3.600, acá puede que exista un cambio de estructura de la información, pero también puede que exista una correlación escalonada con factores exógenos. Para entender mejor esto, voy a trabajar con algunas transformaciones 

Lo primero es convertir los datos en una serie de tiempo, paso seguido descomponer la serie a ver que insigths podemos extraer de los comportamientos individualizados y pot último es determinar que tipo de modelo aplicar. Todo lo anterior lo desarrollaré en un solo código.



```{r}
Serie%>%
  arrange(Fecha)%>%
  select(Cierre)%>%
  ts()%>%
  mstl()%>%
  autoplot()

```

Particularmente la estacionalidad y la tendencia de los componentes parece indicar presencia de una correlación fuerte entre los períodos de tiempo, pero a la par no dejo de sentir algo de ruido y se evidencia una tendencia al alza de manera no constante!

Es interesante ver este comportamiento, pues muestra que en efecto un modelo de memoria corta podría ser el indicado.

```{r}
Serie%>%
  arrange(Fecha)%>%
  select(Cierre)%>%
  ts()%>%
  ggtsdisplay()
```

Increíble los datos pierden potencia de correlación con su pasado, hay que ser cauto a la hora de tratar la serie, a demás todo parece indicar que existe un modelo AR (1), más tengo mis reservas aun, puede que se trate de un factor de integración simplemente y que realmente la serie tenga un comportamiento de caminata aleatoria por los cambios de nivel.


Voy a trabajar con la serie diferenciada a ver si encuentro la tendencia que es lo que más llama mi atención

```{r}
Serie%>%
  arrange(Fecha)%>%
  select(Cierre)%>%
  ts()%>%
  diff()%>%
  mstl()%>%
  autoplot()
```
Bueno, en efecto hay una tendencia bastante fuerte [-20,20], pero este efecto por la magnitud que tiene confirma mi sospecha de caminata aleatoria, for la fuerza del despliegue que tiene la curva, lo cual me lleva a concluir que existe una constante de integración o en términos técnicos un **Random walk with drift**

Gracias a todos los insigths que tenemos, ya se puede construir un modelo ARIMA, de la siguiente manera


```{r}
Serie_ts<-Serie%>%
  arrange(Fecha)%>%
  select(Cierre)%>%
  ts()
  
sarima(Serie_ts,p=0,d=1,q=0)
```

La evidencia indica que este es el modelo adecuado, los errores tiene un buen desempeño. Pero para concluirlo este es un modelo de Ruido Blanco

Por lo tanto paso hacer el modelo para pronosticar.


```{r, echo=FALSE, cache=FALSE}
Serie_ts%>%
  ets()%>%
  forecast::forecast(h=1)%>%
  autoplot()+
  labs(title = 'Pronóstico Dolar')
```
El dolar el día 12 de Junio cerro en \$3.776,50  y el modelo pronóstico \$3.778,499

## Explorando resultados

Al principio sospeche que existía un AR(1), en el modelo, y la evidencia arroja que es un ARIMA (0,1,0), que es lo mismo que ruido, por lo tanto dada la predicción real del dólar evaluare qué tan real es la idea del Autoregresivo.
```{r}

trend <- seq_along(Serie_ts)
(fit1 <- auto.arima(Serie_ts, d=0, xreg=trend))

(fit2<-auto.arima(Serie_ts,d=1))
fc1 <- forecast(fit1,
  xreg = length(Serie_ts) + 1:4)
fc2 <- forecast(fit2, h=4)
autoplot(Serie_ts) +
  autolayer(fc2, series="Tendencia estocástica") +
  autolayer(fc1, series="Tendencia determinístico")+
  labs(title = 'Comparación del modelo determinístico vs estocástico')
```

En efecto hay una tendencia AR1, y la predicción del dolar bajo este foco fue 99.6% acertada.


Esto fue todo por esta oportunidad, espero que les haya gustado!

## Una apuesta más

Dejo los datos del modelo determinístico para ver qué tanto se ajustan los valores con la realidad.

```{r, echo=FALSE, cache=FALSE}
fc1%>%
  knitr::kable(caption = 'Dolar con el modelo determinístico')
```

Y acá dejo los valores de la predicción a través de una red neuronal

```{r, echo=FALSE, cache=FALSE}
nnetar(Serie_ts,lambda = 0)%>%
  forecast(h=4)
```

```{r, echo=FALSE, cache=FALSE}
nnetar(Serie_ts,lambda = 0)%>%
  forecast(h=4)%>%
  autoplot()+
  labs(title = 'Frecasting a través de una red neuronal')
```





[^1]: https://es.wikipedia.org/wiki/Regresi%C3%B3n_local
